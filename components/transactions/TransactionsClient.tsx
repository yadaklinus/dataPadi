...
import {
    ...
    Tv, Zap, CreditCard, Receipt, Clock, GraduationCap, ChevronRight
} from 'lucide-react';
import { getTransactionHistory } from '@/app/actions/user';
import { CURRENCY } from '@/constants';
import { Transaction, TransactionType } from '@/types/types';

// Dynamic import for the detail modal to reduce initial load
const TransactionDetailsModal = dynamic(() => import('@/components/modals/TransactionDetailsModal'), { ssr: false });
...
const TransactionsClient: React.FC<TransactionsClientProps> = ({ initialTransactions, initialFilter = 'All' }) => {
    ...
    // Only run if filter changes after initial mount
    useEffect(() => {
    ...
    const getTransactionConfig = (type: string) => {
        switch (type) {
            ...
            case TransactionType.WALLET_FUNDING:
                return { icon: <CreditCard size={20} />, bg: 'bg-orange-50', color: 'text-orange-600' };
